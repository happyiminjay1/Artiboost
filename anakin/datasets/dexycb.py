...
def plot_vertices(vertices,filename):
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')

    # Assuming vertices is of shape (1, N, 3)
    xs = vertices[0, :, 0].numpy()
    ys = vertices[0, :, 1].numpy()
    zs = vertices[0, :, 2].numpy()
    ...
    plt.title('3D Vertex Visualization')
...
class DexYCB(HOdata):
        ...
        if self.use_left_hand and not self.filter_no_contact and not self.filter_invisible_hand:
            ...
            if self.use_cache and os.path.exists(self.cache_path):
                ...
                for i, sample in enumerate(etqdm(self.dataset)):
                    ...
                    #['color_file', 'depth_file', 'label_file', 'intrinsics', 'ycb_ids', 
                    #'ycb_grasp_ind', 'mano_side', 'mano_betas', 'taxonomy', 'grasped', 'scene_id', 'im_id', 
                    #'is_bop_target', 'is_grasp_target']
    ...
    def get_hand_verts_3d(self, idx):
        ...
        # plot_vertices(hand_verts,'pre.png')

        # hand_verts[:, 0] = -hand_verts[:, 0]

        # plot_vertices(hand_verts,'after.png')
    ...
    def get_image(self, idx):
        ...
        # img.save(f'./TestOutput/original_{idx}.jpg')
    ...
    def get_taxonomy(self, idx):
        sample = self.dataset[idx]
        return sample["taxonomy"]

    def get_ycb_ids(self, idx):
        sample = self.dataset[idx]
        return sample["ycb_ids"]

    def get_im_id(self, idx):
        sample = self.dataset[idx]
        return sample["im_id"]
    ...
    def get_grasped(self, idx):
        sample = self.dataset[idx]
        return sample["grasped"]
