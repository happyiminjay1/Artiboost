...
class HOdata(ABC):
    ...
    def __getitem__(self, idx):
        ...
        sample[Queries.OBJ_VERTS_3D] = self.get_obj_verts_transf(idx)
        sample[Queries.HAND_SHAPE] = self.get_hand_shape(idx)
        sample[Queries.HAND_POSE] = self.get_hand_pose(idx)
        sample[Queries.HAND_VERTS_3D] = self.get_hand_verts_3d(idx)
