...
def mano_de(params, joint_root, bone):
    ...
    #print(so3)
    #print(so3[npj.newaxis, ...])

    #print('npj',npj.newaxis)
    #npj : None
...
class FittingUnit(object):
    ...
    def __call__(self, inp, pred_joints):
        ...
        for batch_id in range(batch_size):
            ...
            so3_ =  params["so3"] # 48
            beta_ = params["beta"] # 10
            bone_ = params["bone"] # 10
            ...
            v_list.append(v)
            j_list.append(j)
            so3_list.append(so3_)
            beta_list.append(beta_)
            bone_list.append(bone_)
        ...
        return v_list, j_list, so3_list, beta_list, bone_list